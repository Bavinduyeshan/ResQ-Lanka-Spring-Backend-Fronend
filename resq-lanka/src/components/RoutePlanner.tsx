...
import L from 'leaflet';
import 'leaflet/dist/leaflet.css';
import { X, Home, Users, Phone, MapPin, CheckCircle, AlertTriangle, XCircle, Wrench } from 'lucide-react';
...
function nearestShelters(district: string, shelters: Shelter[], n = 3): (Shelter & { distKm: number })[] {
  ...
  return shelters
      .map(s => {
        const coords = DISTRICT_COORDS[s.district];
        const distKm = coords ? haversineKm(origin, coords) : 9999;
        return { ...s, distKm };
      })
...
const SHELTER_STATUS_META: Record<ShelterStatus, { color: string; bg: string; label: string; dot: string; tailwindColor: string; tailwindBg: string; tailwindBorder: string }> = {
...
// ── Custom DOM event name used as React↔Leaflet bridge ──
const OPEN_SHELTER_EVENT = 'dmc:open-shelter';

// ── Shelter Detail Modal (pure React) ──────────────────────────────────────
function ShelterDetailModal({
  ...
  return (
      ...
      >
        <div className="bg-white rounded-3xl shadow-2xl border border-slate-100 w-full max-w-md overflow-hidden animate-fade-in">

          {/* ── Modal header ── */}
          <div className="px-6 pt-6 pb-4 border-b border-slate-100 flex items-start justify-between gap-3">
            <div className="flex items-center gap-3">
              <div className="w-11 h-11 rounded-2xl bg-indigo-50 flex items-center justify-center text-2xl shadow-sm border border-indigo-100">
                🏠
              </div>
              <div>
                <h2 className="font-extrabold text-slate-800 text-base leading-tight">{shelter.shelterName}</h2>
                <div className="flex items-center gap-1.5 mt-1 text-xs text-slate-400">
                  <MapPin className="w-3 h-3" />
                  <span>{shelter.district}{shelter.address ? ` · ${shelter.address}` : ''}</span>
                </div>
              </div>
            </div>
            <button
          ...
          <div className="px-6 py-5 space-y-5">
            ...
            <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold border ${meta.tailwindColor} ${meta.tailwindBg} ${meta.tailwindBorder}`}>
              ...
              {(distKm !== undefined || shelter.distKm !== undefined) && (
                  <span className="text-xs font-semibold text-slate-500 bg-slate-100 px-3 py-1.5 rounded-full">
                ~{distKm ?? shelter.distKm} km from incident
              </span>
              )}
            </div>
            ...
            <div>
              ...
              <div className="flex justify-between mt-1.5 text-[11px] text-slate-400">
                ...
                {avail > 0 ? `${avail.toLocaleString()} available` : 'No space'}
              </span>
              </div>
            </div>
                ...
                <div className="bg-slate-50 border border-slate-100 rounded-2xl p-4 flex items-center justify-between">
                  {shelter.contactPerson && (
                      <div className="flex items-center gap-2 text-sm text-slate-600">
                        <div className="w-7 h-7 rounded-lg bg-blue-50 border border-blue-100 flex items-center justify-center text-base">👤</div>
                  ...
                  {shelter.contactPhone && (
                      <div className="flex items-center gap-2 text-sm text-slate-600">
                        ...
                        <span className="font-semibold tabular-nums">{shelter.contactPhone}</span>
                      </div>
                  )}
                </div>
            )}
...
function MapLayer({ incidents, shelters, pathResult, nearestCount, onOpenShelter }: MapLayerProps) {
  ...
  // Keep a ref to onOpenShelter so Leaflet click handlers always use the latest version
  const onOpenShelterRef = useRef(onOpenShelter);
  useEffect(() => { onOpenShelterRef.current = onOpenShelter; }, [onOpenShelter]);

  // Listen for the custom DOM event fired by Leaflet popup HTML buttons
  useEffect(() => {
    const handler = (e: Event) => {
      const customEvent = e as CustomEvent<{ shelterId: number; distKm: number }>;
      const { shelterId, distKm } = customEvent.detail;
  ...
  useEffect(() => {
    if (!styleRef.current) {
      ...
      style.textContent = `
        ...
        .path-line-animated { animation: dash-flow 0.6s linear infinite; }
        .shelter-btn-popup {
          width: 100%;
          text-align: left;
          background: #f8fafc;
        ...
        .shelter-btn-popup:hover {
          background: #eff6ff;
          border-color: #bfdbfe;
        }
        .shelter-btn-popup:last-child { margin-bottom: 0; }
      `;
      document.head.appendChild(style);
      styleRef.current = style;
    }
  ...
  // Helper: build the shelter button HTML inside incident popup
  function buildShelterButtonHtml(s: Shelter & { distKm: number }): string {
    ...
    return `
      ...
      >
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:4px">
          ...
          <span style="font-size:9px;font-weight:700;padding:2px 6px;border-radius:99px;background:${sm.bg};color:${sm.color};border:1px solid ${sm.color}33;">${sm.label}</span>
        </div>
        <div style="font-size:10px;color:#94a3b8;margin-bottom:5px">📍 ${s.district} · ~${s.distKm} km away</div>
        ...
        <div style="display:flex;justify-content:space-between;font-size:9px;color:#94a3b8">
          <span>${pct}% full (${s.occupancy.toLocaleString()}/${s.capacity.toLocaleString()})</span>
          <span style="color:${avail > 0 ? '#16a34a' : '#dc2626'};font-weight:600">${avail > 0 ? avail.toLocaleString() + ' free' : 'Full'}</span>
        </div>
        <div style="margin-top:5px;font-size:9px;color:#6366f1;font-weight:600;">Tap to view full details →</div>
      </button>`;
  }

  // Draw incident markers + shelter markers
  useEffect(() => {
    if (incidentLayerRef.current) {
      incidentLayerRef.current.clearLayers();
    } else {
    ...
    for (const [district, distIncidents] of Object.entries(byDistrict)) {
      ...
      // Incident popup rows
      const popupRows = distIncidents.map((inc, idx) => {
        const incCfg = STATUS_CONFIG[inc.status];
        const score  = inc.severity * inc.affectedPeople;
        const icon   = TYPE_ICON[inc.disasterType] ?? '⚠️';
        return `<div style="border-top:${idx === 0 ? 'none' : '1px solid #f1f5f9'};padding:${idx === 0 ? '0' : '8px 0 0 0'};">
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:4px">
            <span style="font-weight:600;font-size:13px">${icon} ${inc.disasterType}</span>
            <span style="font-size:10px;font-weight:700;padding:2px 7px;border-radius:99px;background:${incCfg.color}22;color:${incCfg.color}">${inc.status}</span>
          </div>
      ...
      // Nearby shelters section (each is a clickable button)
      const nearby = nearestShelters(district, shelters, nearestCount);
      const shelterButtonsHtml = nearby.length === 0
          ? `<div style="font-size:11px;color:#94a3b8;font-style:italic;padding:4px 0">No shelters registered.</div>`
          : nearby.map(s => buildShelterButtonHtml(s)).join('');

      const nearbySection = `
        <div style="border-top:2px solid #f1f5f9;margin-top:10px;padding-top:10px">
          <div style="font-size:10px;font-weight:800;text-transform:uppercase;letter-spacing:0.08em;color:#6366f1;margin-bottom:8px">
      ...
      circle.bindPopup(`
        <div style="min-width:260px;max-width:300px;font-family:system-ui,sans-serif">
          <div style="font-weight:700;font-size:14px;color:#1e293b;margin-bottom:8px;padding-bottom:8px;border-bottom:2px solid #f1f5f9">
            📌 ${district}
            ${distIncidents.length > 1 ? `<span style="font-size:11px;background:#dbeafe;color:#1d4ed8;padding:2px 7px;border-radius:99px;margin-left:6px;font-weight:600">${distIncidents.length} incidents</span>` : ''}
          </div>
    ...
    // ── Shelter markers (standalone, clickable) ───────────────────
    const sheltersByDistrict: Record<string, (Shelter & { distKm: number })[]> = {};
    for (const s of shelters) {
      if (!DISTRICT_COORDS[s.district]) continue;
      if (!sheltersByDistrict[s.district]) sheltersByDistrict[s.district] = [];
      // distKm = 0 here since these are standalone markers (not relative to an incident)
      sheltersByDistrict[s.district].push({ ...s, distKm: 0 });
    }

    for (const [district, distShelters] of Object.entries(sheltersByDistrict)) {
      const [lat, lng] = DISTRICT_COORDS[district];
      const offsetLat = lat + 0.04;
      const offsetLng = lng + 0.04;

      const rep  = distShelters.find(s => s.status === 'OPEN') ?? distShelters[0];
      ...
      const shelterIcon = L.divIcon({
        ...
        html: `
          ...
          ">
            🏠
            ${distShelters.length > 1 ? `<div style="position:absolute;top:-5px;right:-5px;width:14px;height:14px;background:#6366f1;color:white;border-radius:99px;font-size:8px;font-weight:800;display:flex;align-items:center;justify-content:center;border:1.5px solid white">${distShelters.length}</div>` : ''}
          </div>`,
        iconSize:   [22, 22],
        iconAnchor: [11, 11],
      });

      // Build popup with clickable rows for each shelter in this district
      const allRows = distShelters.map((s, i) => {
        const sm    = SHELTER_STATUS_META[s.status];
        const pct   = s.capacity > 0 ? Math.min(100, Math.round((s.occupancy / s.capacity) * 100)) : 0;
        const avail = s.capacity - s.occupancy;
        const barC  = pct >= 95 ? '#ef4444' : pct >= 75 ? '#f59e0b' : '#22c55e';
      ...
      L.marker([offsetLat, offsetLng], { icon: shelterIcon, zIndexOffset: 300 })
          .bindPopup(`
          <div style="min-width:250px;max-width:290px;font-family:system-ui,sans-serif">
            <div style="font-weight:700;font-size:14px;color:#1e293b;margin-bottom:8px;padding-bottom:8px;border-bottom:2px solid #f1f5f9;display:flex;align-items:center;gap:7px">
              <span style="font-size:18px">🏠</span>
              <div>
                ${district} Shelters
                ${distShelters.length > 1 ? `<div style="font-size:10px;font-weight:600;color:#6366f1;margin-top:1px">${distShelters.length} facilities registered</div>` : ''}
              </div>
            </div>
            ${allRows}
          </div>`, { maxWidth: 310 })
  ...
  useEffect(() => {
    ...
    if (!pathResult || pathResult.path.length < 2) return;
    const { path } = pathResult;
    const latlngs = path.filter(d => DISTRICT_COORDS[d]).map(d => DISTRICT_COORDS[d] as [number, number]);

    L.polyline(latlngs, { color: '#1d4ed8', weight: 10, opacity: 0.18 }).addTo(pathLayerRef.current!);
    L.polyline(latlngs, { color: '#2563eb', weight: 4,  opacity: 0.85 }).addTo(pathLayerRef.current!);
    ...
    path.forEach((district, idx) => {
      ...
      if (isEndpoint) {
        const endIcon = L.divIcon({
          className: '',
          html: `<div style="width:20px;height:20px;border-radius:50%;background:${isFrom ? '#16a34a' : '#dc2626'};border:3px solid white;box-shadow:0 0 0 2px ${isFrom ? '#16a34a' : '#dc2626'},0 2px 8px rgba(0,0,0,0.3);display:flex;align-items:center;justify-content:center;font-size:10px;">${isFrom ? '🟢' : '🔴'}</div>`,
        ...
        L.circleMarker(coords, { radius: 5, fillColor: '#2563eb', fillOpacity: 1, color: 'white', weight: 2 })
            .bindTooltip(district, { permanent: false, direction: 'top' }).addTo(pathLayerRef.current!);
      }
      const stepIcon = L.divIcon({
        className: '',
        html: `<div style="font-size:9px;font-weight:800;color:white;background:#2563eb;border-radius:99px;padding:1px 5px;white-space:nowrap;box-shadow:0 1px 4px rgba(0,0,0,0.2);pointer-events:none;">${idx + 1}</div>`,
        iconAnchor: [0, isEndpoint ? 24 : 18],
      });
      L.marker(coords, { icon: stepIcon, interactive: false, zIndexOffset: 600 }).addTo(pathLayerRef.current!);
    });
...
export default function IncidentMap({ incidents = [], shelters = [], height = '520px' }: IncidentMapProps) {
  ...
  return (
      <div className="space-y-4">
        ...
        {/* Shelter detail modal — rendered at root level, above everything */}
        {selectedShelter && (
            <ShelterDetailModal
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h2 className="text-lg font-bold text-slate-800">Live Incident Map</h2>
            <p className="text-sm text-slate-500 mt-0.5">
              All declared incidents plotted on Sri Lanka · click a marker for details &amp; nearest shelters
            </p>
          </div>
          <div className="flex flex-wrap gap-2 text-xs font-semibold">
            <span className="flex items-center gap-1.5 px-3 py-1.5 bg-red-50 text-red-700 rounded-full border border-red-100">
            ...
            <span className="flex items-center gap-1.5 px-3 py-1.5 bg-indigo-50 text-indigo-700 rounded-full border border-indigo-100">
              🏠 {shelterOpenCount} Shelters Open
            </span>
          </div>
        </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm space-y-4">
          ...
          {/* Nearby shelters control */}
          <div className="flex items-center gap-3 pb-4 border-b border-slate-100">
            <span className="text-sm font-semibold text-slate-600 shrink-0">🏠 Show nearest</span>
            ...
            <span className="text-xs text-slate-400">per incident district popup</span>
          </div>
          ...
          <div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 items-end">
              <div className="flex-1">
                <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1.5">🟢 From District</label>
                <select
                    value={fromDistrict}
                    onChange={e => { setFromDistrict(e.target.value); setPathResult(null); setNoPath(false); }}
                    className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 bg-slate-50 text-slate-800 text-sm focus:bg-white focus:border-blue-300 focus:outline-none transition-colors"
              ...
              <button
                  onClick={() => { setFromDistrict(toDistrict); setToDistrict(fromDistrict); setPathResult(null); }}
                  className="px-3 py-2.5 rounded-xl border border-slate-200 hover:bg-slate-50 text-slate-500 text-sm transition-colors shrink-0 cursor-pointer"
                  title="Swap from/to"
              >⇄</button>
              <div className="flex-1">
                <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1.5">🔴 To District</label>
                <select
                    value={toDistrict}
                    onChange={e => { setToDistrict(e.target.value); setPathResult(null); setNoPath(false); }}
                    className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 bg-slate-50 text-slate-800 text-sm focus:bg-white focus:border-blue-300 focus:outline-none transition-colors"
            ...
            {pathResult && (
                <div className="mt-4 p-4 bg-blue-50 border border-blue-100 rounded-xl">
                  <div className="flex items-center justify-between flex-wrap gap-2">
                    <div className="flex items-center gap-2">
                      <span className="text-blue-700 font-bold text-sm">Shortest Path Found</span>
                      <span className="text-xs bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full font-semibold">{pathResult.distanceKm} km</span>
                      <span className="text-xs bg-slate-100 text-slate-600 px-2 py-0.5 rounded-full font-semibold">{pathResult.path.length} districts</span>
                    </div>
                  </div>
                  <div className="mt-2 flex flex-wrap items-center gap-1 text-xs text-slate-600">
                    {pathResult.path.map((d, i) => (
                        <React.Fragment key={d}>
                          <span className={`px-2 py-0.5 rounded-md font-semibold ${i === 0 ? 'bg-green-100 text-green-700' : i === pathResult.path.length - 1 ? 'bg-red-100 text-red-700' : 'bg-slate-100 text-slate-600'}`}>{d}</span>
                          {i < pathResult.path.length - 1 && <span className="text-slate-300 font-bold">→</span>}
                        </React.Fragment>
                    ))}
                  </div>
                </div>
        ...
        <div className="rounded-2xl border border-slate-100 shadow-sm overflow-hidden" style={{ height }}>
          {incidents.length === 0 && shelters.length === 0 && !pathResult ? (
              ...
              <MapContainer key="incident-map" center={[7.8731, 80.7718]} zoom={7} style={{ width: '100%', height: '100%' }} scrollWheelZoom>
                <TileLayer
                    attribution='&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'
                    url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
                />
        ...
        <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex flex-wrap items-center gap-6 text-xs text-slate-500">
          ...
          <span className="flex items-center gap-2 pl-2 border-l border-slate-200">
            ...
            Relief shelter — click for details
          </span>
          <span className="flex items-center gap-2 pl-2 border-l border-slate-200">
            <span className="w-8 h-1 bg-blue-600 inline-block rounded" />
            Shortest path route
